...
/**
 * page-resolver.ts — v7 FINAL
 *
 * Two exported functions:
 *  resolvePageId(url)       → { pageId, pageName, method }
 *  findPageDisplayName(slug) → "Gangnam Beauty Center" from slug "gangnambeautycenter1"
 *
...
// ─── findPageDisplayName ──────────────────────────────────────────────────────
/**
 * CRITICAL: Converts slug → real display name for Apify search.
 * "gangnambeautycenter1" → "Gangnam Beauty Center"
  ...
  try {
    ...
    for (const item of organic) {
      ...
      if (cleaned && cleaned.length > 2 && cleaned.length < 80 &&
        ...
        console.log(`[page-resolver] ✅ displayName found: "${cleaned}" for slug "${slug}"`);
        return cleaned;
      }
    }
  } catch (err) {
  ...
  for (const q of queries) {
    try {
      ...
      for (const item of organic) {
        ...
        if (m?.[1]) {
          const title = safeStr(item.title) ?? "";
          const pageName = title.replace(/\s*[|–\-]\s*.*$/i, "").trim() || undefined;
          console.log(`[page-resolver] ✅ M4 Serper: ${m[1]} (${pageName})`);
...
export async function resolvePageId(
  ...
  if (userToken) {
    const r = await tryGraphApi(slug, userToken, opts.graphVersion ?? "v25.0");
    if (r) { console.log(`[page-resolver] ✅ M2 → ${r.pageId} (${r.pageName})`); return { ...r, method: "graph_user_token" }; }
    console.log(`[page-resolver] M2 failed`);
  } else {
  ...
  if (appId && appSecret) {
    ...
    const r = await tryGraphApi(slug, `${appId}|${appSecret}`, opts.graphVersion ?? "v25.0");
    if (r) { console.log(`[page-resolver] ✅ M3 → ${r.pageId} (${r.pageName})`); return { ...r, method: "graph_app_token" }; }
    console.log(`[page-resolver] M3 failed`);
  } else {
  ...
  if (serperKey) {
    console.log(`[page-resolver] M4 Serper strict...`);
    const r = await trySerperAdLibrary(slug, serperKey);
    if (r) { console.log(`[page-resolver] ✅ M4 → ${r.pageId}`); return { ...r, method: "serper_ad_library" }; }
    console.log(`[page-resolver] M4 failed`);
  } else {
  ...
  console.log(`[page-resolver] M5 Ad Library scrape...`);
  const r5 = await tryAdLibraryScrape(slug);
  if (r5) { console.log(`[page-resolver] ✅ M5 → ${r5.pageId}`); return { ...r5, method: "ad_library_scrape" }; }

  // M6: HTML scrape
  console.log(`[page-resolver] M6 HTML scrape...`);
  const r6 = await tryHtmlScrape(url, slug);
  if (r6) { console.log(`[page-resolver] ✅ M6 → ${r6.pageId}`); return { ...r6, method: "html_scrape" }; }
